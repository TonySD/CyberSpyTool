...
public class PortScan {
    ...
    public static List<Port> parseNmapOutput(BufferedReader outputStream) throws IOException {
        ...
        while (!(outputLine = outputStream.readLine()).isBlank()) {
            if (Character.isDigit(outputLine.charAt(0))) {
                if (currentPort != null) ports.add(currentPort);
                matcher = portInfoPattern.matcher(outputLine);
                if (matcher.find()) {
